...
BOOL CRsaEngine::CSPInit(bool bUseExistKeyPairs, LPTSTR szPublicKeyFile, LPTSTR szPrivateKeyFile)
{
	...
	//打开或创建CSP容器
	if (!CryptAcquireContext(&m_hCryptProv, m_szCspName, NULL, PROV_RSA_FULL, 0))//默认设置为0，获取出错则新建
	{
		if (GetLastError() == NTE_BAD_KEYSET)//容器不能被打开
		{
			if (!CryptAcquireContextW(&m_hCryptProv, m_szCspName, NULL, PROV_RSA_FULL, CRYPT_NEWKEYSET))//使用指定名容器新建
			{
				return FALSE;
			}
		}
